				  ...
				  bool ApplyICHEPAdditionalFilters)
{
  ...
  if (!_ismc && (ApplyGiovanniFilters || _systematic.Contains("ZZ"))) {
                        ...
                        TString suffix)
{
  ...
  if (_sample.Contains("MET"))            _ismc = false;
...
void AnalysisCMS::ShapeWZ()
{
  ...
  int nTightLeptons = 3;
  if (_systematic.Contains("WZloose")) nTightLeptons = 2;
  if (_ntightlepton<nTightLeptons) { _event_weight = 0.; return; }
  ...
  for (int l1 = 0; l1<3; l1++) {
	...
	if (DMZcand<DMZCut && _systematic.Contains("WZlooseZ") && _ntightlepton<3)
	  if (AnalysisLeptons[l1].type==1 && AnalysisLeptons[l2].type==1) { _event_weight = 0.; return; }
...
// ShapeZZ
//------------------------------------------------------------------------------
void AnalysisCMS::ShapeZZ()
{
  
  if (_nlepton<4) { _event_weight = 0.; return; }
  ...
  if (_systematic.Contains("ZZtightEl") && jetpt1>25.) { _event_weight = 0.; return; }
  ...
  if (DMZT>900. || Z1lep1<0 || Z1lep2<0 || Z2lep1<0 || Z2lep2<0) { 
    ...
    TVector3 NewMET; NewMET.SetXYZ(MET.Px() + AnalysisLeptons[Z2lep1].v.Px() + AnalysisLeptons[Z2lep2].v.Px(),
				   MET.Py() + AnalysisLeptons[Z2lep1].v.Py() + AnalysisLeptons[Z2lep2].v.Py(),
				   MET.Pz());
    MET.SetPtEtaPhiM(NewMET.Pt(), 0.0, NewMET.Phi(), 0.0);
...
void AnalysisCMS::ShapeWZforZZ()
{
  ...
  if (DMZ>=DMZCut || Zlep1<0 || Zlep2<0 || Lostlep<0) { 
    ...
    AnalysisLeptons[2] = AnalysisLeptons[Lostlep];
...
void AnalysisCMS::EventSetup(float jet_eta_max, float jet_pt_min)
{
  ...
  //----------------------------------------------------------------------------
  //if (_analysis.EqualTo("Stop") && _systematic.Contains("ZWtoZ")) ShapeWZforZZ();
  //else if (_analysis.EqualTo("Stop") && _systematic.Contains("WZ")) ShapeWZ();
  //else if (_analysis.EqualTo("Stop") && _systematic.Contains("ZZ")) ShapeZZ();
...
void AnalysisCMS::OpenMinitree()
{
  ...
  if (_systematic.Contains("multilepton")) {
    minitree->Branch("lep3id",            &_lep3id,           "lep3id/F");
    minitree->Branch("lep3type",          &_lep3type,         "lep3type/F");
    minitree->Branch("lep3eta",           &_lep3eta,          "lep3eta/F");
  ...
  minitree->Branch("trig_SnglMu",       &trig_SnglMu,       "trig_SnglMu/F");
  minitree->Branch("trig_SnglEle",      &trig_SnglEle,      "trig_SnglEle/F");
  ...
  if (_analysis.EqualTo("Stop") && _sample.Contains("ZZTo") &&  !_sample.Contains("qqHToZZ")) {
    minitree->Branch("ZZmass",               &_ZZmass,              "Zmass/F");
    minitree->Branch("ZZdphi",               &_ZZdphi,              "Zdphi/F");
    minitree->Branch("ZZpt",                 &_ZZpt,                "Zpt/F");
  }
...
void AnalysisCMS::GetZZGenVar()
{
  ...
  if (nZZLeps!=4) cout << "AnalysisCMS::GetZZGenVar: warning " << nZZLeps << " leptons found" << endl; 
	...
	for (int l2 = 0; l2<nZZLeps; l2++) {
	  if (l2!=l0 && l2!=l1) {
		...
		if (fabs(ZZLepID[l2])==fabs(ZZLepID[l3]) && (ZZLepID[l2]*ZZLepID[l3]<0. || ZZLepID[l2]%2==0)) {
		  ...
		  float ZZDiff = sqrt( pow(Z0.M()-Z_MASS, 2) +  pow(Z1.M()-Z_MASS, 2) );
		  if (ZZDiff<MinZZDiff) {

		    TLorentzVector ZZCand = Z0 + Z1;
		    
		    _ZZmass = ZZCand.M();
		    _ZZpt   = ZZCand.Pt();
		    _ZZdphi = Z1.DeltaPhi(Z0);
...
void AnalysisCMS::GetStopVar()
{
  ...
  for (int ml = 0; ml<2; ml++) {
	...
	if (MinimumDeltaR < 0.04 && lepIndex[ml] < 0) lepIndex[ml] = lp; // different use
      ...
      if (DeltaRGenLepLep2<MinimumDeltaR && DeltaRGenLepLep2<fabs(_lep2isfake)) {
...
//------------------------------------------------------------------------------
void AnalysisCMS:: ReadGluGluHZZkfactors() 
{
  ...
  TFile* fin = TFile::Open("stop/Kfactor_Collected_ggHZZ_2l2l_NNLO_NNPDF_NarrowWidth_13TeV.root");
...
//------------------------------------------------------------------------------
float AnalysisCMS::kfactor_ggHZZ_qcd(float GENmassZZ, TString SystTitle)
{
  ...
  for (int f=0; f<9; f++) 
    if (SystTitle==strSystTitle[f])
      return ggZZ_kf[f]->Eval(GENmassZZ); ;
...
//------------------------------------------------------------------------------
float AnalysisCMS::kfactor_qqZZ_qcd_dPhi(float GENdPhiZZ, int finalState)
{
...
//------------------------------------------------------------------------------
float AnalysisCMS::kfactor_qqZZ_qcd_M(float GENmassZZ, int finalState)
{
...
//------------------------------------------------------------------------------
float AnalysisCMS::kfactor_qqZZ_qcd_Pt(float GENpTZZ, int finalState)
{
